...
const AppTodo = props => {
	...
	const onFilterSelect = filter => setFiltr(filter);

	let filterPost = (items, filter) => filter === "like" ? items.filter(item => item.like) : items; /// SEARCH


	const [term, setTerm] = useState("");

	let searchPost = (items, term) => term.length === 0 ? items : items.filter(item => item.label.indexOf(term) > -1);
	...
	const onClickDelItem = id => (index = "", index = data.findIndex(elem => elem.id === id), setData([...data.slice(0, index), ...data.slice(index + 1)])); // ADD
	// let maxId = 5;
	...
	const addItem = body => (newItem = {
		...
		like: false,
		id: Date.now()
	}, setData([...data, newItem])); // MARK AS IMPORTANT
	...
	}, setData([...data.slice(0, index), newItem, ...data.slice(index + 1)])); // MARK AS LIKED


	const onToggleLiked = id => (index = "", index = data.findIndex(elem => elem.id === id), oldItem = {}, oldItem = data[index], newItem = { ...oldItem,
		like: !oldItem.like
